...
export function useConvexPRReviewData(user?: UserInfo | null, teamSlug?: string) {
    ...
    const undoLastAssignmentMutation = useMutation(api.mutations.undoLastAssignment);
    const toggleAbsenceMutation = useMutation(api.mutations.toggleReviewerAbsence);
    ...
    const isLoading = reviewers === undefined || nextReviewer === undefined || assignmentFeed === undefined;
    ...
    const addReviewer = async (name: string, email: string) => {
        ...
        try {
            ...
            toast({
                ...
                description: error instanceof Error ? error.message : t("data.addReviewerFailedDescription"),
                variant: "destructive",
            });
            return false;
        }
    ...
    const updateReviewer = async (id: string, name: string, email: string) => {
        ...
        try {
            ...
            toast({
                ...
                description: error instanceof Error ? error.message : t("reviewer.updateFailed"),
                variant: "destructive",
            });
            return false;
        }
