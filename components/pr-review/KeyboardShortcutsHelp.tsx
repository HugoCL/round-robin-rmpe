                ...
                <div className="py-4">
                        ...
                        {shortcuts.map((shortcut) => (
                            <div key={shortcut.key} className="flex items-start justify-between gap-4">
                                <div className="flex-1">
                                    ...
                                    <div className="text-sm text-muted-foreground">{shortcut.note}</div>
                                </div>
                                <div className="flex items-center gap-1">
                                    {shortcut.key.split(" + ").map((key) => (
                                        <span key={key} className="flex items-center">
                                            {key !== shortcut.key.split(" + ")[0] && <span className="mx-1">+</span>}
                    ...
                    <div className="mt-6 p-3 bg-muted rounded-md">
                        <p className="text-sm text-muted-foreground">
                            <strong>Note:</strong> Keyboard shortcuts work globally when this page is active. 
                            Some shortcuts are only available when there's a next reviewer available.
                        </p>
                    </div>
                </div>
            </DialogContent>
